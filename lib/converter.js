...
class Converter {
  ...
  init() {
    this._ready = exec('/usr/bin/python3.5 /usr/bin/unoconv --help')
    .then((result) => {
  ...
  startListener() {
    ...
    this.listener = childProcess.spawn('/usr/bin/python3.5 /usr/bin/unoconv', ['--listener']);
  }
  ...
  convert(inputFile, args, options) {
    ...
    return new Promise((resolve, reject) => {
      const handler = childProcess.spawn('/usr/bin/python3.5 /usr/bin/unoconv', args);
      const errors = [];
